#!/usr/bin/env python3
"""
vibecheck Security Gate - PreToolUse Hook
...
# Inline suppression pattern: // vibecheck-disable CHECK-ID or # vibecheck-disable CHECK-ID
SUPPRESSION_PATTERN = re.compile(
    r'(?://|#|/\*)\s*vibecheck-disable\s+([\w,-]+)', re.IGNORECASE
)
...
# Config file name
CONFIG_FILENAME = '.vibecheck.json'


def load_config(file_path):
    """Load project-level .vibecheck.json config.
...
def is_config_weakening(file_path, content):
    """Detect writes to .vibecheck.json that weaken security (add disabled checks)."""
...
def main():
    ...
    if is_config_weakening(file_path, content):
        print(
            "\n\u26a0\ufe0f  vibecheck: You are disabling security checks in .vibecheck.json.\n"
            "  Make sure this is intentional. Disabled checks will no longer protect your codebase.\n"
            "  Prefer inline suppression (// vibecheck-disable CHECK-ID) for specific cases.\n",
            file=sys.stderr
        )
        # Warn but allow
        sys.exit(0)
    ...
    for result in warned:
        print(
            f"\n\u26a0\ufe0f  vibecheck [{result.check_id}] {result.category}: {result.message}"
            f"\n   Fix: {result.fix_suggestion}"
            f"\n   Suppress: // vibecheck-disable {result.check_id}\n",
            file=sys.stderr
        )

    # Block on CRITICAL findings
    if blocked:
        output = ["\n\U0001f6d1 vibecheck BLOCKED this write. Security vulnerabilities detected:\n"]
        ...
        output.append(
            f"  Suppress if intentional: // vibecheck-disable {','.join(r.check_id for r in blocked)}\n"
        )
